package tech.abstracty.agent.tools

import ai.koog.agents.core.tools.*
import io.ktor.client.*
import io.ktor.client.call.*
import io.ktor.client.request.*
import io.ktor.http.*
import kotlinx.coroutines.*
import kotlinx.serialization.KSerializer
import kotlinx.serialization.Serializable
import kotlinx.serialization.json.JsonObject
import kotlinx.serialization.json.jsonArray
import kotlinx.serialization.json.jsonObject
import org.apache.pdfbox.Loader
import org.apache.pdfbox.text.PDFTextStripper
import org.jsoup.Jsoup
import tech.abstracty.agent.rag.ConnectionPool
import java.net.URI
import java.net.http.HttpRequest
import java.net.http.HttpResponse
import java.time.Duration
import kotlin.math.min

/**
 * Web search tool using Google Programmable Search API.
 *
 * Features:
 * - Performs Google searches via the Custom Search JSON API
 * - Optionally fetches and extracts content from search results
 * - Supports HTML and PDF content extraction
 * - Concurrent fetching with configurable limits
 *
 * Example usage:
 * ```kotlin
 * val searchTool = WebSearchTool(
 *     apiKey = "your-google-api-key",
 *     cx = "your-custom-search-engine-id",
 * )
 *
 * // Add to agent
 * AgentBuilder.create(bridge) {
 *     tools {
 *         +searchTool
 *     }
 * }
 * ```
 */
class WebSearchTool(
    private val apiKey: String,
    private val cx: String,
    private val site: String? = null,
    private val geolocation: String = "NL",
    private val httpClient: HttpClient = ConnectionPool.getKtorClient(),
    private val javaHttpClient: java.net.http.HttpClient = ConnectionPool.getHttpClient(),
) : Tool<WebSearchTool.Args, WebSearchTool.Result>() {

    /**
     * Arguments for web search.
     */
    @Serializable
    data class Args(
        /** The search query */
        val query: String,
        /** Search type (e.g., "image" for image search) */
        val searchType: String? = null,
        /** Whether to fetch and extract page content */
        val fetchContent: Boolean = true,
        /** Maximum number of results to fetch content from */
        val maxFetch: Int = 1,
        /** Maximum characters of fetched text per result */
        val maxChars: Int = 20000,
    ) : ToolArgs

    /**
     * A single search result item.
     */
    @Serializable
    data class Item(
        val title: String,
        val link: String,
        val snippet: String? = null,
        val mime: String? = null,
        val content: String? = null,
        val httpStatus: Int? = null,
        val contentType: String? = null,
    )

    /**
     * Search results.
     */
    @Serializable
    data class Result(
        val totalResults: String?,
        val items: List<Item>,
    ) : ToolResult.JSONSerializable<Result> {
        override fun getSerializer(): KSerializer<Result> = serializer()
    }

    override val argsSerializer = Args.serializer()
    override val resultSerializer: KSerializer<Result> = Result.serializer()
    override val description =
        "Search the web via Google Programmable Search. " +
        "Fetches and extracts HTML or PDF text for the top results."

    override val descriptor = ToolDescriptor(
        name = "WebSearchTool",
        description = description,
        requiredParameters = listOf(
            ToolParameterDescriptor("query", "Query string", ToolParameterType.String)
        ),
        optionalParameters = listOf(
            ToolParameterDescriptor("searchType", "Use 'image' for image search", ToolParameterType.String),
            ToolParameterDescriptor("fetchContent", "Fetch and extract page text", ToolParameterType.Boolean),
            ToolParameterDescriptor("maxFetch", "Number of results to fetch content from", ToolParameterType.Integer),
            ToolParameterDescriptor("maxChars", "Max characters of fetched text", ToolParameterType.Integer)
        )
    )

    override suspend fun execute(args: Args): Result {
        require(apiKey.isNotBlank()) { "Missing Google API key" }
        require(cx.isNotBlank()) { "Missing Google CX" }

        // Build API URL
        val url = URLBuilder("https://www.googleapis.com/customsearch/v1").apply {
            parameters.append("key", apiKey)
            parameters.append("cx", cx)
            parameters.append("q", args.query)
            parameters.append("gl", geolocation)
            args.searchType?.let { parameters.append("searchType", it) }
            site?.let {
                parameters.append("siteSearch", it)
                parameters.append("siteSearchFilter", "i")
            }
        }.buildString()

        // Perform search
        val response = withContext(Dispatchers.IO) { httpClient.get(url) }
        val resp: JsonObject = response.body()

        val total = resp["searchInformation"]
            ?.jsonObject
            ?.get("totalResults")
            ?.toString()
            ?.trim('"')

        // Parse results
        val allItems = resp["items"]
            ?.jsonArray
            ?.map { it.jsonObject }
            ?.map {
                Item(
                    title = it["title"]?.toString()?.trim('"') ?: "",
                    link = it["link"]?.toString()?.trim('"') ?: "",
                    snippet = it["snippet"]?.toString()?.trim('"'),
                    mime = it["mime"]?.toString()?.trim('"')
                )
            } ?: emptyList()

        if (allItems.isEmpty()) {
            return Result(totalResults = total, items = emptyList())
        }

        val max = args.maxFetch.coerceAtLeast(1).coerceAtMost(allItems.size)
        val limitedItems = allItems.take(max)

        // Optionally fetch content
        val fetchedItems = if (args.fetchContent) {
            coroutineScope {
                limitedItems.map { item ->
                    async {
                        val fetched = fetchPage(item.link, args.maxChars)
                        if (fetched?.body != null) {
                            item.copy(
                                content = fetched.body,
                                httpStatus = fetched.status,
                                contentType = fetched.contentType,
                                snippet = null
                            )
                        } else {
                            null
                        }
                    }
                }.awaitAll()
            }.filterNotNull()
        } else {
            limitedItems
        }

        return Result(totalResults = total, items = fetchedItems)
    }

    private suspend fun fetchPage(url: String, maxChars: Int): FetchedText? =
        withContext(Dispatchers.IO) {
            try {
                withTimeout(15_000) {
                    val request = HttpRequest.newBuilder()
                        .uri(URI.create(url))
                        .timeout(Duration.ofSeconds(10))
                        .header("User-Agent", "curl/8.1.2")
                        .header("Accept", "*/*")
                        .GET()
                        .build()

                    val response = javaHttpClient.send(request, HttpResponse.BodyHandlers.ofByteArray())
                    val status = response.statusCode()
                    val contentType = response.headers().firstValue("content-type").orElse("")

                    if (status >= 400) return@withTimeout FetchedText(null, status, contentType)

                    val bodyBytes = response.body()
                    val ct = contentType.lowercase()
                    val extractedText = when {
                        "text/html" in ct -> extractHtmlText(bodyBytes, maxChars)
                        "application/pdf" in ct -> extractPdfText(bodyBytes, maxChars)
                        ct.startsWith("text/") -> String(bodyBytes).take(maxChars)
                        else -> null
                    }

                    FetchedText(extractedText, status, contentType)
                }
            } catch (_: Exception) {
                null
            }
        }

    private fun extractHtmlText(bodyBytes: ByteArray, maxChars: Int): String {
        val doc = Jsoup.parse(String(bodyBytes))
        doc.select("script, style, nav, header, footer, .nav, .menu, .sidebar, #cookie-banner").remove()
        val main = doc.select("main, article, .content, #content, [role=main]").firstOrNull()
        return (main ?: doc.body()).text().replace("\\s+".toRegex(), " ").trim().take(maxChars)
    }

    private fun extractPdfText(bodyBytes: ByteArray, maxChars: Int): String? {
        return try {
            if (bodyBytes.size > 10_000_000) return "Large PDF - skipped"
            Loader.loadPDF(bodyBytes).use { doc ->
                if (doc.isEncrypted) runCatching { doc.setAllSecurityToBeRemoved(true) }
                PDFTextStripper().apply {
                    sortByPosition = true
                    startPage = 1
                    endPage = min(doc.numberOfPages, 10)
                }.getText(doc)
                    .replace("\\s+".toRegex(), " ")
                    .trim()
                    .take(maxChars)
            }
        } catch (_: Exception) {
            null
        }
    }

    private data class FetchedText(val body: String?, val status: Int, val contentType: String?)

    companion object {
        /**
         * Create a web search tool from environment variables.
         *
         * Reads GOOGLE_API_KEY and GOOGLE_CX from the environment.
         */
        fun fromEnvironment(
            site: String? = null,
            geolocation: String = "NL",
        ): WebSearchTool {
            val apiKey = System.getenv("GOOGLE_API_KEY")
                ?: throw IllegalStateException("GOOGLE_API_KEY environment variable not set")
            val cx = System.getenv("GOOGLE_CX")
                ?: throw IllegalStateException("GOOGLE_CX environment variable not set")

            return WebSearchTool(
                apiKey = apiKey,
                cx = cx,
                site = site,
                geolocation = geolocation,
            )
        }
    }
}
